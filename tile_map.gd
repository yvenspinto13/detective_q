...
func mark_and_update_tile(puzzle: String, tile_id: int) ->void:
	...
	if tile_id > 0:
		...
		old_tile.sort_custom(func(a: Vector2i, b: Vector2i) -> bool:
			if a.y != b.y:
				return a.y < b.y
			else:
				return a.x < b.x)
		self.set_cell(1, old_tile[0], puzzle_clueid[puzzle], Vector2i(0, 0))
		self.set_cell(1, old_tile[1], puzzle_clueid[puzzle], Vector2i(1, 0))
		self.set_cell(1, old_tile[2], puzzle_clueid[puzzle], Vector2i(0, 1))
