...
func _ready() -> void:
	...
	spwan_tween.tween_property(self, "position", position + Vector2(0, -16), .4)
	spwan_tween.tween_callback(func (): allow_horizontal_movement = true)

func _process(delta: float) -> void:
	if allow_horizontal_movement:
		position.x += delta * horizontal_speed
		
	#print("collision",shape_cast_2d.is_colliding() )
